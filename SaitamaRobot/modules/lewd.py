...
import SaitamaRobot.modules.sql.hentai_sql as sql
...
def dva(update, context):
    ...
    if not update.effective_message.chat.type == "private":
        ...
        if not is_hentai:
            return
    msg = update.effective_message
    hentai = requests.get("https://api.computerfreaker.cf/v1/dva").json()
    url = hentai.get("url")
...
ADD_hentai_HANDLER = CommandHandler("addhentai", add_hentai)
REMOVE_hentai_HANDLER = CommandHandler("rmhentai", rem_hentai)
LIST_hentai_CHATS_HANDLER = CommandHandler(
    "hentaichats", list_hentai_chats, filters=CustomFilters.dev_filter)
...
NEKOhentai_HANDLER = CommandHandler("nekohentai", nekohentai)
...
dispatcher.add_handler(NEKOhentai_HANDLER)
...
Also thanks to [EverythingSuckz](https://t.me/EverythingSuckz) for hentai filter.
    
Usage:
    
/addhentai : Enable hentai mode
/rmhentai : Disable hentai mode
...
Commands :   
 ...
 - /nekohentai: Sends Random hentai Neko source Images.
 - /solo: Sends Random hentai Neko GIFs.
